...
A "Group by" picker in the list header groups rows by a chosen field, mirroring Payload's native group-by. URL-param state (`?listGroupBy=<field>`, `-field` for descending group order — deliberately not `?groupBy=`, since Payload core's `renderListView` persists its own `groupBy` preference and would fight this feature under that key); selecting **None** clears it.
...
| Collection-level `upload: { ... }` (e.g. Media) | ✓ supported (v3.5) — custom shadcn dropzone above the field list, multipart submit on create / file replace. Client-side direct-to-bucket upload (v3.23) is wired but disabled — broken in `@payloadcms/storage-r2@3.85.1` (see §7b.5 note) |
...
The bulk-edit matrix (`src/features/list-view/bulk/BulkEditFieldInput.tsx`) is intentionally narrower than the doc form — it still only handles the v1 scalar subset. The v2 leaf types (point/code/json/array/blocks/polymorphic relationship) and v3 leaf type (richText) render in the doc form via the same `FieldInput` switch, but `isBulkEditable` keeps them off the bulk-edit sheet.

**Sidebar fields (v3.25).** Top-level fields carrying Payload's `admin.position: 'sidebar'` are pulled out of the main field list into a dedicated right-hand column (`lg:w-72`, left border) — mirroring Payload's own `DocumentFields` split. Only top-level fields are eligible; nested fields are never pulled out, and collections with no sidebar fields render the unchanged single column. See FEATURES.md's `defaultDocView` entry for the full layout details.
...
- **`versions.drafts.localizeStatus: true`** (experimental in Payload 3.85.1; requires `experimental.localizeStatus: true` in payload config). When enabled with multiple locales:
  ...
  - **Segmented locale+status control (v3.25).** When the doc is multi-locale, `DocStatusBar` collapses the separate `LocaleSwitcher` and per-locale pills into a single `h-8` segmented control (a globe icon plus one clickable segment per locale, each showing that locale's status) instead of two adjacent widgets. Single-locale docs, or docs without `localizeStatus`, still fall back to the plain single pill.
