...
/* RSC installed at `admin.components.views.account` by the `defaultAuthViews`
   ...
   `payload-plugin-shadcn-admin/rsc#AutoAccountView`. */
export async function AutoAccountView({ initPageResult }: AdminViewServerProps) {
  ...
  if (!collectionConfig || !user) {
    return (
      <ViewShell
        ...
        breadcrumbs={[{ label: i18n.t('authentication:account') }]}
      >
  ...
  let doc: Record<string, unknown> = user as Record<string, unknown>
  try {
    ...
    if (fetched && typeof fetched === 'object') {
      doc = fetched as Record<string, unknown>
    }
  } catch {
  ...
  const supportedLanguages = (payload.config.i18n?.supportedLanguages ??
    {}) as Record<string, { translations?: { general?: { thisLanguage?: string } } }>
  ...
  let docPermissions: unknown = permissions?.collections?.[userSlug]
  try {
  ...
  return (
    ...
    >
      <AccountForm
        ...
        initialApiKey={
          typeof doc.apiKey === 'string' ? doc.apiKey : null
        }
