...
/* Field-level type:'upload' input.
   ...
   "Upload new" drawer creates into whichever slug is currently active. */
...
// Polymorphic envelope used both on the wire (Payload's storage shape) and
// inside this component for tracking which slug each picked doc belongs to.
type PolyEntry = { value: string; relationTo: string }

const normalizeNonPoly = (
  value: unknown,
  hasMany: boolean,
): string | string[] | null => {
  ...
  if (Array.isArray(value)) {
    return value
      .map((v) =>
        typeof v === 'object' && v && 'id' in (v as any)
          ? String((v as any).id)
          : String(v),
      )
      .filter(Boolean)
...
const normalizePoly = (value: unknown): PolyEntry[] => {
  if (value === null || value === undefined) return []
  const arr = Array.isArray(value) ? value : [value]
  const out: PolyEntry[] = []
  for (const item of arr) {
    if (item && typeof item === 'object') {
      const entry = item as { value?: unknown; relationTo?: unknown; id?: unknown }
      if (
        (typeof entry.value === 'string' || typeof entry.value === 'number') &&
        typeof entry.relationTo === 'string'
      ) {
        out.push({ value: String(entry.value), relationTo: entry.relationTo })
      } else if (
        (typeof entry.id === 'string' || typeof entry.id === 'number') &&
        typeof entry.relationTo === 'string'
      ) {
        // Some populated Payload reads include the full doc with relationTo.
        out.push({ value: String(entry.id), relationTo: entry.relationTo })
      }
    }
  }
  return out
...
const collectIds = (v: string | string[] | null): string[] => {
  if (v === null) return []
...
export function UploadFieldInput({
  ...
  disabled,
  invalid,
}: UploadFieldInputProps): React.ReactElement {
  ...
  const polySlugs = isPoly ? (relationTo as string[]) : []
  ...
  // relationTo.
  const polyEntries = React.useMemo(
    () => (isPoly ? normalizePoly(value) : []),
    [isPoly, value],
  )
  const nonPolyNormalized = React.useMemo(
    () => (isPoly ? null : normalizeNonPoly(value, Boolean(hasMany))),
    [isPoly, value, hasMany],
  )

  const [activeSlug, setActiveSlug] = React.useState<string>(
    isPoly
      ? polyEntries[0]?.relationTo ?? polySlugs[0] ?? ''
  ...
  React.useEffect(() => {
    if (!isPoly) {
      setActiveSlug(relationTo as string)
      return
    }
  ...
  }, [isPoly, polySlugs.join(','), polyEntries.length])
  ...
  const allEntries: PolyEntry[] = isPoly
    ? polyEntries
    : collectIds(nonPolyNormalized).map((id) => ({
        value: id,
  ...
  React.useEffect(() => {
    ...
    void (async () => {
      type FetchResult = { slug: string; docs: any[] } | null
      ...
      for (const [slug, ids] of bySlug.entries()) {
        ...
        const p: Promise<FetchResult> = fetch(
          ...
          .then(async (res): Promise<FetchResult> => {
            ...
            return { slug, docs: data.docs ?? [] }
          })
          .catch((): FetchResult => null)
  ...
  const handlePick = (picked: string | string[] | null) => {
    if (isPoly) {
      ...
      const entry: PolyEntry = { value: String(pickedId), relationTo: activeSlug }
      if (!hasMany) {
        onChange(entry)
        return
      }
      const exists = polyEntries.some(
        (e) => e.relationTo === entry.relationTo && e.value === entry.value,
      )
      if (exists) return
      onChange([...polyEntries, entry])
      return
    }
    ...
    onChange(Array.isArray(picked) ? picked : [picked])
  }

  const removeEntry = (target: PolyEntry) => {
    if (isPoly) {
      if (!hasMany) {
        onChange(null)
        return
      ...
      onChange(
        polyEntries.filter(
          (e) => !(e.relationTo === target.relationTo && e.value === target.value),
        ),
      )
      return
    }
  ...
  const handleUploadSuccess = React.useCallback(
    (created: UploadCreated[]) => {
      ...
      if (isPoly) {
        const newEntries: PolyEntry[] = created.map((c) => ({
          value: String(c.id),
          relationTo: c.slug,
        }))
        if (!hasMany) {
        ...
        const dedup = new Map<string, PolyEntry>()
    ...
    [hasMany, isPoly, polyEntries, nonPolyNormalized, onChange],
  )
    ...
    <div className="flex flex-col gap-2">
          ...
          {allEntries.map((entry) => {
            ...
            return (
              <div
                key={key}
                className={cn(
                  'group relative flex items-center gap-2 rounded-md border bg-card p-2',
                  invalid && 'border-destructive',
                )}
              >
                <div className="flex size-12 shrink-0 items-center justify-center overflow-hidden rounded border bg-background">
                  {isImg && src ? (
                ...
                <div className="min-w-0 max-w-[12rem]">
                  ...
                  <p className="truncate text-xs font-medium">
                    {doc?.filename ?? entry.value}
                  </p>
                  <p className="truncate text-[10px] text-muted-foreground">
                    {doc?.mimeType ?? entry.value}
                  </p>
                </div>
                <button
                  type="button"
      ...
      <div className="flex items-center gap-2">
        {isPoly ? (
          ...
          >
            <SelectTrigger className="w-40" aria-label="Collection">
