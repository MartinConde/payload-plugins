/* The relationship / upload pickers surface ids as strings (they key search
   ...
   `{ relationTo, value }` envelopes (single or array). */

// Only coerce when the integer round-trips exactly: this leaves leading-zero
...
const coerceId = (v: unknown): unknown =>
  typeof v === 'string' && /^\d+$/.test(v) && String(Number(v)) === v
    ? Number(v)
    : v

export const coerceRelationshipValue = (v: unknown): unknown => {
  if (Array.isArray(v)) return v.map(coerceRelationshipValue)
  if (v !== null && typeof v === 'object' && 'value' in v) {
    return { ...(v as Record<string, unknown>), value: coerceId((v as Record<string, unknown>).value) }
  }
  return coerceId(v)
}
