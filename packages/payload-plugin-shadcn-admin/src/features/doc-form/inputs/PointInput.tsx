...
/* Point field input. Payload stores points as GeoJSON [lng, lat] on disk and
   ...
   numbers (or null if both fields are blank). */
...
const stringifyCoord = (v: unknown): string => {
  if (typeof v === 'number' && Number.isFinite(v)) return String(v)
  if (typeof v === 'string') return v
  return ''
}

const incomingFromValue = (value: unknown): { lng: string; lat: string } => {
  if (Array.isArray(value) && value.length >= 2) {
    return { lng: stringifyCoord(value[0]), lat: stringifyCoord(value[1]) }
  }
  return { lng: '', lat: '' }
}
...
export function PointInput({
  ...
  invalid,
  disabled,
}: PointInputProps): React.ReactElement {
  const incoming = incomingFromValue(value)
  const [lngText, setLngText] = React.useState<string>(incoming.lng)
  const [latText, setLatText] = React.useState<string>(incoming.lat)
  ...
  React.useEffect(() => {
    ...
    const lngMismatch =
      typeof lngParsed === 'number'
        ? next.lng !== String(lngParsed)
        : next.lng !== '' || lngText === ''
    const latMismatch =
      typeof latParsed === 'number'
        ? next.lat !== String(latParsed)
        : next.lat !== '' || latText === ''
  ...
  const emit = (nextLngText: string, nextLatText: string) => {
