...
/* shadcn date picker (Popover + Calendar) replacing the native
   ...
   primitive, so a styled native time input is the simplest correct control. */
...
/* Local "HH:mm" for the time input — built from local hours/minutes so it
   round-trips with what the user sees in the formatted trigger. */
...
export function DateInput({
  ...
  const emit = (d: Date | null) => onChange(d ? d.toISOString() : null)
      ...
      <PopoverTrigger asChild>
        ...
        >
          ...
          {label ?? (
            <span>
              {withTime
                ? t('shadcnAdmin:pickDateTime')
                : `${t('shadcnAdmin:pickDate')}…`}
            </span>
          )}
        </Button>
      </PopoverTrigger>
