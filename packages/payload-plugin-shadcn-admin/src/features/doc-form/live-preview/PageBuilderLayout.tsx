...
/* Owns the doc form's content-area layout below the sticky toolbar: the
   ...
   the block-action handlers, the add-block-picker state) lives here. */
...
import { subPerms, type Perms } from '../access-control/fieldPermissions.js'
...
import { LivePreviewPanel, type PageBuilderBlockAction } from './LivePreviewPanel.js'
...
export function PageBuilderLayout({
  ...
  selectedBlockId,
  setSelectedBlockId,
}: PageBuilderLayoutProps): React.ReactElement {
  ...
  const layoutFieldPerms = React.useMemo(
    () =>
      layoutField
        ? (subPerms(docPermissions, layoutField.name) as Perms | undefined)
        : undefined,
    [layoutField, docPermissions],
  )
