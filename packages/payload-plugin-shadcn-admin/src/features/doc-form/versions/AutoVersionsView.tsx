...
import { VersionsList, type VersionRow } from './VersionsList.js'
...
/* Versions LIST view, installed at `admin.components.views.edit.versions` by
   ...
   Mounted as `payload-plugin-shadcn-admin/rsc#AutoVersionsView`. v3.9. */
export async function AutoVersionsView(serverProps: DocumentViewServerProps) {
  const { initPageResult, searchParams, payload } = serverProps
  const t = serverProps.i18n.t as TFunction<ShadcnAdminTranslationsKeys>
  ...
  if (!entity || !entitySlug || (!isGlobal && docID === undefined)) {
    return (
      <ViewShell breadcrumbs={[{ label: t('version:versions') }]}>
  ...
  const and: Record<string, unknown>[] = isGlobal
    ? []
    : [{ parent: { equals: docID } }]
  if (hasLocalization && draftsEnabled) {
    and.push({ snapshot: { not_equals: true } })
  }
  ...
  try {
    ...
    rows = (result.docs ?? []).map((v: any) => ({
      id: String(v.id),
      updatedAt: typeof v.updatedAt === 'string' ? v.updatedAt : '',
      status: v.version?._status ?? null,
      autosave: Boolean(v.autosave),
      publishedLocale:
        typeof v.publishedLocale === 'string' ? v.publishedLocale : null,
    }))
  } catch {
  ...
  let basePath: string
  const baseCrumbs: { label: string; href?: string }[] = []
  if (isGlobal) {
    editTitle =
      stringifyLabel((global as any).label, serverProps.i18n) ??
    ...
    editTitle =
      useAsTitle && doc && typeof doc[useAsTitle] === 'string'
        ...
        : String(docID)
    pluralLabel =
      stringifyLabel((collection as any).labels?.plural, serverProps.i18n) ??
      titleCase(entitySlug)
    basePath = `/admin/collections/${entitySlug}/${docID}`
    baseCrumbs.push(
      { label: t('general:collections') },
      { label: pluralLabel, href: `/admin/collections/${entitySlug}` },
      { label: editTitle, href: basePath },
    )
  }
  ...
  return (
    <ViewShell
      ...
      breadcrumbs={[...baseCrumbs, { label: t('version:versions') }]}
    >
      <VersionsList
        rows={rows}
        basePath={basePath}
