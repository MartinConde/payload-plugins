/* Pure helpers for the schedule-publish surface. Server-safe (no React / DOM
   ...
   app's jobs queue (`jobs.autoRun` / external cron). */
...
export const getSchedulePublishConfig = (
  collection: ExtractedCollection,
): { timeFormat?: string; timeIntervals?: number } | null => {
...
export const wallClockToInstant = (wallClock: Date, timezone: string): Date => {
  const tz = new TZDate(
