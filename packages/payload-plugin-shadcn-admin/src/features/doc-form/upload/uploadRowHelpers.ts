/* Pure helpers for UploadNewDialog's per-row bookkeeping: form-state-fetch
   ...
   React — kept separate so the dialog file stays focused on orchestration. */

import { isFieldRenderable } from '../fieldTree/sharedHelpers.js'
...
export const FORM_STATE_TYPES = new Set(['richText', 'array', 'blocks'])

export const schemaHasFormStateFields = (fields: ExtractedField[]): boolean => {
...
export const topLevelRequiredLeafNames = (fields: ExtractedField[]): string[] => {
  const out: string[] = []
