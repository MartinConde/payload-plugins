/* Field-type support matrix for the auto doc form.
   ...
   regardless of single-vs-poly `relationTo`). */

import type { ExtractedField } from 'payload-plugin-shadcn-ui'

export const SUPPORTED_DOC_FORM_TYPES = new Set([
  'text',
  'textarea',
  'email',
  'number',
...
/* Structural containers that the bridge walks transparently (their direct
   children are what the matrix actually checks). */
const STRUCTURAL_PASSTHROUGH = new Set(['row', 'collapsible', 'group'])

export const isFieldSupportedForDocForm = (field: ExtractedField): boolean => {
...
/* Walk a Payload collection (raw or extracted) and return any required fields
   ...
   create / when a new file is picked. */
export const findBlockingRequiredFields = (
  collection: { fields?: any[]; upload?: unknown },
): { name: string; type: string }[] => {
  const blockers: { name: string; type: string }[] = []
  const visit = (fields: any[] | undefined, prefix: string) => {
    ...
    for (const f of fields) {
      ...
      if (t === 'tabs') {
        for (const tab of f.tabs ?? []) {
          const tabPrefix =
            tab && typeof tab.name === 'string' ? `${prefix}${tab.name}.` : prefix
      ...
      blockers.push({ name: `${prefix}${f.name ?? '(unnamed)'}`, type: t })
    }
  }
