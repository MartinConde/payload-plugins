...
/* Persists per-collection column order + visibility to Payload's preferences
   ...
   actively change something before we start persisting again. */
...
const prefKey = (collectionSlug: string): string =>
  `collection-list-columns-${collectionSlug}`
...
export function useColumnPrefs(collectionSlug: string): UseColumnPrefsReturn {
  ...
  const pendingRef = React.useRef<{ order: string[] | undefined; visibility: Visibility } | null>(
    null,
  )
  ...
  const findPrefDoc = React.useCallback(
    async (
      key: string,
    ): Promise<{ id: string; value: StoredPref | null } | null> => {
  ...
  const writePrefDoc = React.useCallback(
    async (key: string, value: StoredPref): Promise<void> => {
  ...
  const deletePrefDoc = React.useCallback(async (): Promise<void> => {
  ...
  React.useEffect(() => {
    ...
    void (async () => {
      ...
      setState({
        ...
        visibility:
          stored.visibility && typeof stored.visibility === 'object'
            ? stored.visibility
            : {},
        loaded: true,
      })
  ...
  }, [collectionSlug, writePrefDoc])

  const scheduleWrite = React.useCallback(
    (next: { order: string[] | undefined; visibility: Visibility }) => {
  ...
  const setOrder = React.useCallback(
    (next: string[]) => {
      ...
      setState((prev) => {
        ...
        scheduleWrite({ order: next, visibility: prev.visibility })
        return updated
      })
    },
    [scheduleWrite],
  ...
  const setVisibility = React.useCallback(
    (next: Visibility) => {
      interactedRef.current = true
      setState((prev) => {
        const updated = { ...prev, visibility: next }
        scheduleWrite({ order: prev.order, visibility: next })
        return updated
      })
    },
    [scheduleWrite],
  ...
  const reset = React.useCallback(() => {
    ...
    void deletePrefDoc()
    setState({ order: undefined, visibility: {}, loaded: true })
  }, [deletePrefDoc])
