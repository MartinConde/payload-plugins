...
const prefKey = (collectionSlug: string): string =>
  `collection-list-filters-${collectionSlug}`
...
/* Pending (value-less) chips don't belong in persisted prefs — they're
   ...
   ghost chip with no value. */
const chipIsPersistable = (chip: FilterChip): boolean => {
...
const stripValuelessChips = (state: FilterState): FilterState => {
  ...
  for (const node of state.nodes) {
    ...
    nodes.push({ kind: 'group', group: { ...node.group, chips } })
  }
  return { nodes }
}

const urlHasFiltersNow = (): boolean => {
...
export function usePreferencesSync({
  ...
  const pendingRef = React.useRef<FilterState | null>(null)
  ...
  }, [collectionSlug, findPrefDoc, interactedRef])
