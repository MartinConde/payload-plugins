...
/* URL state for the where-builder filter bar. Mirrors useDataTableUrlState's
   ...
   chips deduplicated against URL chips that share their (field, operator). */
...
const chipHasMaterialValue = (chip: Pick<FilterChip, 'operator' | 'value'>): boolean => {
...
const collectChips = (state: FilterState): FilterChip[] => {
...
const findChip = (
  ...
  | { nodeIndex: number; groupChipIndex: number | null; chip: FilterChip }
  | null => {
  for (let i = 0; i < state.nodes.length; i += 1) {
    const node = state.nodes[i]
    if (node.kind === 'chip' && node.chip.id === id) {
      return { nodeIndex: i, groupChipIndex: null, chip: node.chip }
    }
    if (node.kind === 'group') {
...
const mapChip = (
  state: FilterState,
  id: string,
  fn: (chip: FilterChip) => FilterChip,
): FilterState => ({
  nodes: state.nodes.map((node) => {
    if (node.kind === 'chip') {
      if (node.chip.id !== id) return node
      return { kind: 'chip', chip: fn(node.chip) }
    }
    const idx = node.group.chips.findIndex((c) => c.id === id)
    if (idx === -1) return node
    const nextChips = node.group.chips.map((c, i) => (i === idx ? fn(c) : c))
    return { kind: 'group', group: { ...node.group, chips: nextChips } }
  }),
})

const dropChip = (state: FilterState, id: string): FilterState => {
  const nextNodes: FilterNode[] = []
  for (const node of state.nodes) {
    if (node.kind === 'chip') {
    ...
    if (chips.length === 1) {
      nextNodes.push({ kind: 'chip', chip: chips[0] })
    } else {
      nextNodes.push({ kind: 'group', group: { ...node.group, chips } })
    }
  }
  return { nodes: nextNodes }
}

const normalize = (state: FilterState): FilterState => {
  // Collapse single-chip OR groups back to flat chips
  const nodes: FilterNode[] = []
  for (const node of state.nodes) {
    if (node.kind === 'group' && node.group.chips.length === 1) {
      nodes.push({ kind: 'chip', chip: node.group.chips[0] })
    } else {
      nodes.push(node)
    }
  }
...
export function useFilterUrlState(): UseFilterUrlStateResult {
  ...
  const urlState = React.useMemo<FilterState>(() => {
  ...
  const [pendingChips, setPendingChips] = React.useState<FilterChip[]>([])
  ...
  React.useEffect(() => {
    setPendingChips((prev) => {
      ...
      const filtered = prev.filter(
        (p) => !present.has(`${p.field}|${p.operator}`),
      )
  ...
  const state = React.useMemo<FilterState>(() => {
    ...
    const seenFieldOps = new Set<string>()
    for (const node of urlState.nodes) {
      if (node.kind === 'chip') {
    ...
    const visiblePending: FilterChip[] = []
    ...
    return {
      nodes: [
        ...urlState.nodes,
        ...visiblePending.map((chip) => ({ kind: 'chip' as const, chip })),
      ],
    }
  }, [urlState, pendingChips])
  ...
  /* Anticipate the URL chip ID a pending chip will receive once promoted, so
     ...
     this (field, op) + already-anticipated pending chips + this one. */
  const anticipateChipId = React.useCallback(
    (
      field: string,
      operator: string,
      excludePendingId: string | null = null,
    ): string => {
  ...
  const writeUrlState = React.useCallback(
    (next: FilterState) => {
  ...
  const addChip = React.useCallback<UseFilterUrlStateResult['addChip']>(
    (chip) => {
      markInteracted()
      const id = anticipateChipId(chip.field, chip.operator)
      const fullChip: FilterChip = { ...chip, id }
      if (chipHasMaterialValue(fullChip)) {
        writeUrlState({
          nodes: [...urlState.nodes, { kind: 'chip', chip: fullChip }],
        })
      } else {
        setPendingChips((prev) => [...prev, fullChip])
      }
      return id
    },
    [urlState, writeUrlState, anticipateChipId, markInteracted],
  )

  const updateChip = React.useCallback(
    (id: string, patch: Partial<Omit<FilterChip, 'id'>>) => {
      markInteracted()
      const pendingIdx = pendingChips.findIndex((c) => c.id === id)
      if (pendingIdx !== -1) {
        const current = pendingChips[pendingIdx]
        const merged: FilterChip = { ...current, ...patch, id: current.id }
        const fieldOrOpChanged =
          (patch.field !== undefined && patch.field !== current.field) ||
          (patch.operator !== undefined && patch.operator !== current.operator)
        if (fieldOrOpChanged) {
        ...
        if (chipHasMaterialValue(merged)) {
          ...
          writeUrlState({
            nodes: [
              ...urlState.nodes.filter(
                (n) =>
                  !(
                    n.kind === 'chip' &&
                    n.chip.field === merged.field &&
                    n.chip.operator === merged.operator
                  ),
              ),
              { kind: 'chip', chip: merged },
            ],
          })
        }
        return
      }
      const next = mapChip(state, id, (c) => ({ ...c, ...patch }))
      writeUrlState(next)
    },
    [pendingChips, urlState, state, writeUrlState, anticipateChipId, markInteracted],
  )
  ...
  const removeChip = React.useCallback(
    ...
    [pendingChips, state, writeUrlState, markInteracted],
  )
  ...
  const moveChip = React.useCallback(
    (id: string, direction: -1 | 1) => {
      ...
      const nodes = [...urlState.nodes]
      ;[nodes[i], nodes[j]] = [nodes[j], nodes[i]]
      writeUrlState({ nodes })
    },
    [pendingChips, urlState, writeUrlState, markInteracted],
  )

  const toggleOrJoin = React.useCallback(
    (id: string) => {
      markInteracted()
      if (pendingChips.some((c) => c.id === id)) return
      const found = findChip(urlState, id)
      ...
      if (found.groupChipIndex !== null) {
        ...
        const chips = [...node.group.chips]
        const [chip] = chips.splice(found.groupChipIndex, 1)
        const nodes = [...urlState.nodes]
        if (chips.length === 0) {
          nodes.splice(i, 1)
        } else if (chips.length === 1) {
          nodes[i] = { kind: 'chip', chip: chips[0] }
        } else {
          nodes[i] = { kind: 'group', group: { ...node.group, chips } }
        }
        const insertAt = chips.length === 0 ? i : i + 1
        nodes.splice(insertAt, 0, { kind: 'chip', chip })
        writeUrlState({ nodes })
        return
      }
      ...
      if (prev.kind === 'group') {
        nodes[i - 1] = {
          kind: 'group',
          group: { ...prev.group, chips: [...prev.group.chips, node.chip] },
        }
        nodes.splice(i, 1)
      } else {
        nodes.splice(i - 1, 2, {
  ...
  const hasAnyFilter =
    urlState.nodes.length > 0 || pendingChips.length > 0
