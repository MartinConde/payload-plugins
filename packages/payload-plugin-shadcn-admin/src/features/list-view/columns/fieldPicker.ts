/* Server-callable helpers that inspect a collection's field config. Kept
   ...
   can call them directly. No React, no JSX, no side effects. */
...
/* Project only the picked columns for the list-view refetch. `pickFieldNames`
   ...
   sync with the rendered columns by construction — do not hardcode. */
...
/* Trim populated relationship/upload docs to just their useAsTitle (keyed by
   ...
   footgun. Returns undefined when there's nothing to populate. */
export function buildListPopulate(
  collection: CollectionMeta,
...
/* True when any picked column is a non-polymorphic relationship/upload —
   ...
   useAsTitle of the related document. */
export function collectionNeedsDepthOne(collection: CollectionMeta): boolean {
