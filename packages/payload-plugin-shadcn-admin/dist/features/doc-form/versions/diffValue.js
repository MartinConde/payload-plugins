...
const optionLabel = (options, value)=>{
    ...
    const match = options?.find((o)=>typeof o === 'string' ? o === v : o.value === v);
    if (!match) return v;
    return typeof match === 'string' ? match : match.label;
};
/** Pull a human title out of a populated relationship/upload value. Falls back
 *  to the id (or the raw value) when nothing better is available. */ const relationshipTitle = (value)=>{
...
/* ── richText structural diff (v3.24) ──────────────────────────────────────
   ...
   like uploads/blocks), so an unrecognized node never breaks the diff. */ // Lexical text-format bitmask → wrapping tags (innermost-last).
const FORMAT_TAGS = [
    [
        1,
        'strong'
...
const lexicalNodeToHTML = (node)=>{
    ...
    const childrenHTML = Array.isArray(node.children) ? node.children.map(lexicalNodeToHTML).join('') : '';
    switch(node.type){
        case 'paragraph':
        ...
        case 'autolink':
            {
                const url = typeof node.fields?.url === 'string' ? node.fields.url : undefined;
                return url ? `<a href="${escapeForDiff(url)}">${childrenHTML}</a>` : childrenHTML;
            }
        case 'horizontalrule':
            return '<hr />';
        ...
        case 'upload':
            return `<p>[upload: ${escapeForDiff(String(node.value?.filename ?? node.value?.id ?? node.relationTo ?? ''))}]</p>`;
        case 'relationship':
            return `<p>[relationship: ${escapeForDiff(String(node.value?.id ?? node.relationTo ?? ''))}]</p>`;
        case 'block':
            return `<p>[block: ${escapeForDiff(String(node.fields?.blockType ?? ''))}]</p>`;
        default:
 ...
 *  Text content is escaped; structural/format tags are literal. */ export const richTextToDiffHTML = (value)=>{
...
/** Plain-text serialization of a Lexical richText value. */ const lexicalToText = (value)=>{
    const root = value?.root;
    if (!root || !Array.isArray(root.children)) {
 ...
 *  display string. Used for both From and To sides of the diff. */ export const stringifyDiffValue = (field, value)=>{
    ...
    switch(field.type){
        ...
        case 'select':
        case 'radio':
            return Array.isArray(value) ? value.map((v)=>optionLabel(field.options, v)).join(', ') : optionLabel(field.options, value);
        case 'relationship':
        case 'upload':
            return Array.isArray(value) ? value.map(relationshipTitle).filter(Boolean).join(', ') : relationshipTitle(value);
        case 'point':
