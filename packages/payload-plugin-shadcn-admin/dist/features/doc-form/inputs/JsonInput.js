...
/* JSON field input. Payload's REST endpoint expects a parsed JSON value on
   ...
   the PATCH/POST body, so the marker never reaches the wire. */ import * as React from 'react';
import { Textarea } from 'payload-plugin-shadcn-ui';
export const JSON_PARSE_ERROR_KEY = '__jsonParseError__';
export const isJsonParseError = (v)=>typeof v === 'object' && v !== null && JSON_PARSE_ERROR_KEY in v;
const stringify = (v)=>{
...
export function JsonInput({ id, value, onChange, required, invalid, disabled }) {
    ...
    const [text, setText] = React.useState(()=>isJsonParseError(value) ? value.raw : stringify(value));
    const [parseError, setParseError] = React.useState(isJsonParseError(value) ? value[JSON_PARSE_ERROR_KEY] : null);
