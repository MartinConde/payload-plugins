...
/* One file's create form, rendered through the shared field-tree renderer so
   ...
   (not an inline map callback) so the per-row hooks obey the rules of hooks. */ import * as React from 'react';
import { collectLocalizedSchemaPaths, getByPath, isObject, isRenderableHere, projectLocaleAtLeaves, setByPath, stripPathIndices } from '../fieldTree/sharedHelpers.js';
...
export function UploadRowForm({ rowId, collectionSlug, collectionFields, values, errors, onValuesChange, onErrorClear, useAsTitleBySlug, uploadCollectionsBySlug, activeLocale, localizationEnabled, disabled }) {
