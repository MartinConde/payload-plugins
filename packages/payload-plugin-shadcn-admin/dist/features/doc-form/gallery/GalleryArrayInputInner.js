...
/* The actual gallery field input — a drag-and-drop thumbnail grid where every
   entry has an image (from an upload collection) and a per-entry caption
   rendered via renderChild (so localization + validation are preserved).

   Loaded exclusively via `GalleryArrayInput.tsx`'s `React.lazy(() =>
   import(...))` — never imported directly. This file is where the actual
   `@payloadcms/ui` (via payloadAdapterUI.js) and dnd-kit imports live, which
   is exactly why it must NOT be the thing `galleryField()` value-imports;
   see GalleryArrayInput.tsx's header comment.

   This component is registered (indirectly, via the lazy shell) as a
   `custom['plugin-shadcn-admin'].input` override on a Payload `array` field
   (via `galleryField()` in `src/fields/galleryField.ts`). It receives the
   full FieldInputProps contract from the bridge and replaces the default
   ArrayInput card list with a compact grid UI.
...
// ── main component ────────────────────────────────────────────────────────────
export function GalleryArrayInputInner({ field, value, onChange, nestedPath = '', renderChild, useAsTitleBySlug, uploadCollectionsBySlug = {}, fieldPerms, disabled }) {
