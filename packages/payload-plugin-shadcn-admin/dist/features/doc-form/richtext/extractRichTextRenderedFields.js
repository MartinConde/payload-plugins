/* Lift Payload's pre-rendered richText Field elements out of serverProps.formState.
   ...
   composition so the keys align exactly. */ const isObject = (v)=>typeof v === 'object' && v !== null && !Array.isArray(v);
function* walkRichTextPaths(fields, data, prefix) {
        ...
        if (f.type === 'array') {
            ...
            if (Array.isArray(rows)) {
                for(let idx = 0; idx < rows.length; idx++){
                    yield* walkRichTextPaths(f.fields, rows[idx], `${prefix}${f.name}.${idx}.`);
                }
            }
            continue;
        }
        if (f.type === 'blocks') {
            ...
            if (Array.isArray(rows)) {
                for(let idx = 0; idx < rows.length; idx++){
                    ...
                    if (!block) continue;
                    yield* walkRichTextPaths(block.fields, row, `${prefix}${f.name}.${idx}.`);
                }
            }
            continue;
        }
...
/* Build the richText path → rendered-entry map from the extracted schema, the
   runtime doc data, and Payload's pre-built formState. */ export function extractRichTextRenderedFields(collection, data, formState) {
    if (!formState) return {};
    const out = {};
