/* Pure helpers over `ExtractedCollection.versions` for the auto doc form's
   drafts UI. Server-safe (no React, no DOM). v3.6. */ /** Default autosave debounce — matches Payload's own default. */ export const DEFAULT_AUTOSAVE_INTERVAL_MS = 800;
export const hasDraftsEnabled = (collection)=>{
 ...
 *  null otherwise. Falls back to {@link DEFAULT_AUTOSAVE_INTERVAL_MS}. */ export const getAutosaveInterval = (collection)=>{
    const v = collection.versions;
    if (!v || v.drafts === false || v.drafts === undefined) return null;
    const a = v.drafts.autosave;
    if (a === false || a === undefined) return null;
    return typeof a.interval === 'number' && a.interval > 0 ? a.interval : DEFAULT_AUTOSAVE_INTERVAL_MS;
};
 ...
 *    covers the draft path). */ export const shouldShowSaveDraftButton = (collection)=>{
    const v = collection.versions;
