...
/* Persists the dashboard's widget layout (order, size, hidden) to Payload's
   ...
   are discarded" convention as usePreferencesSync. */ import * as React from 'react';
...
export function useDashboardLayoutPrefs() {
    ...
    const findPrefDoc = React.useCallback(async ()=>{
    ...
    const writePrefDoc = React.useCallback(async (value)=>{
    ...
    const setHidden = React.useCallback((next)=>{
        setState((prev)=>{
            const updated = {
                ...prev,
                hidden: next
    ...
    const setSize = React.useCallback((id, size)=>{
        setState((prev)=>{
            const nextSizes = {
                ...prev.sizes,
                [id]: size
