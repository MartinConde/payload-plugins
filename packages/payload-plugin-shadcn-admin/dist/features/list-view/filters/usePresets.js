...
/* Saved filter presets — named snapshots of the where/sort/search URL params
   ...
   where-builder's indexed shape and any per-column flat shape uniformly. */ import * as React from 'react';
import { usePathname, useRouter, useSearchParams } from 'next/navigation';
const SCHEMA_VERSION = 1;
const MAX_PRESETS = 20;
const prefKey = (collectionSlug)=>`collection-list-presets-${collectionSlug}`;
const isWhereKey = (key)=>key === 'where' || key.startsWith('where[');
const isCapturedKey = (key)=>isWhereKey(key) || key === 'sort' || key === 'search';
/* Sentinel error codes the UI can branch on without inspecting message text. */ export const PRESET_ERROR = {
...
export function usePresets(collectionSlug) {
    ...
    /* Cached prefs doc id, shared between mount-time hydration and writes.
     ...
     should POST), string = existing doc id (next write PATCHes). */ const prefDocIdRef = React.useRef(null);
    const key = prefKey(collectionSlug);
    const findPrefDoc = React.useCallback(async (lookupKey)=>{
    ...
    const writePrefDoc = React.useCallback(async (lookupKey, value)=>{
    ...
    /* Hydrate on mount (per collectionSlug). */ React.useEffect(()=>{
        let cancelled = false;
        setLoaded(false);
        ...
        void (async ()=>{
            ...
            if (stored && stored.schemaVersion === SCHEMA_VERSION && Array.isArray(stored.presets)) {
    ...
    const savePreset = React.useCallback(async (rawName, options)=>{
        /* Guard against the hydration race: until `loaded` flips true, our
         ...
         new preset and silently destroy whatever else was stored. */ if (!loaded) throw new PresetError(PRESET_ERROR.NotReady);
        ...
        const next = isOverwrite ? presets.map((p, i)=>i === existingIdx ? preset : p) : [
            ...presets,
            preset
        ];
        setPresets(next);
    ...
    const loadPreset = React.useCallback((id)=>{
        ...
        /* Drop everything we own: any where[*] key, sort, search, page.
         ...
         alone because it's a viewing preference, not part of the snapshot. */ const toDelete = [];
        params.forEach((_, k)=>{
    ...
    const deletePreset = React.useCallback(async (id)=>{
