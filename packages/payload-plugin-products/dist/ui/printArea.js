 ...
 */ export const IDENTITY_MOCKUP_TRANSFORM = {
    x: 0,
    y: 0,
    scale: 1,
    locked: false
 ...
 *  also allows free custom mm entry. */ export const A_SERIES_PRESETS = [
    {
        label: 'A6',
        widthMm: 105,
        heightMm: 148
...
/** Physical aspect ratio (width / height) of a print area, in mm. */ export const aspectOf = (a)=>a.heightMm > 0 ? a.widthMm / a.heightMm : 1;
/** Physical aspect ratio (width / height) from view dims. Mirrors `aspectOf`
 *  for the geometry-split world. */ export const aspectFromDims = (d)=>d.heightMm > 0 ? d.widthMm / d.heightMm : 1;
/** Project a legacy PrintArea down to its geometry-only placement. */ export const placementFromPrintArea = (a)=>({
        id: a.id,
        name: a.name,
        x: a.x,
        y: a.y,
 ...
 *  for the unchanged Fabric editor. */ export const printAreaFromPlacement = (p, dims)=>({
        id: p.id,
        name: p.name,
 ...
 *  new placement sanitizers. */ const normalizeGeometry = (raw)=>{
...
/** Coerce one stored entry into a clean PrintArea, or null if unusable. */ const normalizeArea = (raw)=>{
    if (!isObj(raw)) return null;
...
/** Sanitize a single geometry-only placement, returning null if unusable. */ export const normalizePlacement = (raw)=>normalizeGeometry(raw);
/** Sanitize an array of placements, dropping any unusable entries. */ export const normalizePlacements = (raw)=>{
    if (!Array.isArray(raw)) return [];
    return raw.map(normalizeGeometry).filter((p)=>p !== null);
};
 ...
 *  Out-of-range values are clamped, not rejected. */ export const normalizeMockupTransform = (raw)=>{
    if (!isObj(raw)) return {
        ...IDENTITY_MOCKUP_TRANSFORM
    };
    return {
 ...
 *  undefined/Dates/functions — safe for the D1 (SQLite) json text column. */ export const normalizePrintAreasValue = (value)=>{
    ...
    const areas = areasRaw.map(normalizeArea).filter((a)=>a !== null);
    return {
 ...
 *  import this. */ export const refToId = (raw)=>{
...
const colorRefToId = (raw)=>{
    const id = refToId(raw);
    return id == null ? null : String(id);
};
const emptyColorMockupRow = (colorId)=>({
 ...
 *  immediately, before the next save runs the server-side reconcile. */ export function reconcileColorMockupsPure(colorIds, views) {
    ...
    return views.map((view)=>{
        ...
        const next = wanted.map((id)=>byColor.get(id) ?? emptyColorMockupRow(id));
        return {
            ...view,
            colorMockups: next
        };
 ...
 */ export function snapPlacementsToAspect(placements, newDims, naturalAspect) {
    ...
    const safeNatural = typeof naturalAspect === 'number' && naturalAspect > 0 && Number.isFinite(naturalAspect) ? naturalAspect : 1;
 ...
 *  (which have their own pixel aspect). */ export const newPrintArea = (preset, natural)=>{
