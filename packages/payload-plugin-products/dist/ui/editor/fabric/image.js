/* Mockup-image helpers — loading and placing a `FabricImage` onto the canvas,
   plus the lock-state toggle. Pure-ish: callers own the Canvas + lifecycle. */ import { FabricImage } from 'fabric';
import { mockupToFabric } from './coords.js';
 ...
 */ export async function loadMockupImage(url, isDisposed, mediaId) {
    try {
        if (mediaId) {
 ...
 *  runs; nothing here mutates anything outside the canvas. */ export function addMockupToCanvas(canvas, img, natural, canvasSize, mt, baselineScale) {
 ...
 *  canvas init. */ export function applyMockupTransform(img, canvasSize, mt, baselineScale) {
    img.set(mockupToFabric(canvasSize, mt, baselineScale));
    img.setCoords();
}
/** Lock / unlock the mockup so it stops responding to drag-select-scale.
 *  Used by the Image tab's "Lock image" toggle. */ export function applyMockupLock(img, locked) {
    img.set({
        selectable: !locked,
        evented: !locked,
