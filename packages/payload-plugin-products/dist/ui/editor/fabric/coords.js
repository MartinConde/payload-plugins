/* Pure coordinate math for the print-area editor — no React, no Fabric.
   ...
   uniform-scaling handler in `events.ts`). */ export const clamp01 = (v)=>v < 0 ? 0 : v > 1 ? 1 : v;
 ...
 *  the canvas top-left at this baseline. */ export const baselineScaleFor = (canvas, natural)=>{
    if (natural.w <= 0 || natural.h <= 0) return 1;
    return Math.min(canvas.width / natural.w, canvas.height / natural.h);
};
/** Effective on-canvas scale of the mockup, after the user's `scale` multiplier. */ export const effectiveMockupScale = (baseline, mt)=>baseline * mt.scale;
/** Mockup top-left in canvas pixel coords. */ export const mockupOriginOnCanvas = (canvas, mt)=>({
        left: canvas.width * mt.x,
        top: canvas.height * mt.y
    });
/** Project an image-local normalized print area to canvas pixel geometry that
 *  can be `.set()` straight onto a Fabric Rect. */ export const areaToRectGeometry = (area, natural, canvas, mt, baseline)=>{
 ...
 *  the natural-image denominator. */ export const rectToArea = (rect, meta, natural, canvas, mt, baseline)=>{
    const effScale = effectiveMockupScale(baseline, mt);
    const { left: mLeft, top: mTop } = mockupOriginOnCanvas(canvas, mt);
...
/** Mockup's canvas-coord placement, for `.set()` on the mockup FabricImage. */ export const mockupToFabric = (canvas, mt, baseline)=>{
    const effScale = effectiveMockupScale(baseline, mt);
    return {
        left: canvas.width * mt.x,
        top: canvas.height * mt.y,
 ...
 *  normalized `MockupTransform` shape we persist. */ export const mockupFromFabric = (fab, canvas, baseline, locked)=>{
