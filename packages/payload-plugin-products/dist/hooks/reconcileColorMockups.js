import { reconcileColorMockupsPure, refToId } from '../ui/printArea.js';
/* Collection beforeChange on products: align each view's `colorMockups[]`
   ...
   and existing rows are preserved across the rebuild. */ export const reconcileColorMockups = ({ data })=>{
    if (!data || typeof data !== 'object') return data;
    const colorsRaw = Array.isArray(data.colors) ? data.colors : [];
    const colorIds = colorsRaw.map(refToId).filter((id)=>id !== null).map(String);
