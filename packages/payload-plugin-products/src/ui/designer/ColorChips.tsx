...
/* Chip strip below ViewTabs. One chip per color in `data.colors` (top-level
   ...
   tab's `colors` relationship field — flagged as a rough edge for Phase 4. */
...
import { reconcileColorMockupsPure, type ColorMockupRow, type PrintAreaPlacement, type ViewRow } from '../printArea.js'
...
export function ColorChips({
  ...
  colorSwatchesSlug,
  disabled,
}: ColorChipsProps): React.ReactElement | null {
  ...
  const lockedRaw = useDocFormFieldValue(
    `views.${viewIndex}.colorMockups.${activeColor}.placementLocked`,
  )

  const colorIds = React.useMemo<string[]>(() => {
  ...
  }, [colorIds.length, activeColor, onActiveColor])
  ...
  const [docsById, setDocsById] = React.useState<Map<string, ColorDoc>>(new Map())
  React.useEffect(() => {
    ...
    void (async () => {
      try {
        const res = await fetch(
          `${apiBase}/${colorSwatchesSlug}?${params.toString()}`,
          { credentials: 'include' },
        )
        if (!res.ok || cancelled) return
        const json = (await res.json()) as { docs?: ColorDoc[] }
        const map = new Map<string, ColorDoc>()
  ...
  }, [idsKey, apiBase, colorSwatchesSlug, colorIds])

  const handleAddExisting = React.useCallback(
    (next: string | string[] | null) => {
      if (!next) return
      const incoming = Array.isArray(next) ? next : [next]
      const merged = Array.from(new Set([...colorIds, ...incoming.map(String)]))
    ...
    [colorIds, setValueAtPath, viewsRaw, onActiveColor],
  )
  ...
  const toggleLock = React.useCallback(() => {
    ...
    const choice = (() => {
      ...
      return window.confirm(
        tr('pluginProducts:unlockDialogDescription') +
          '\n\n' +
          tr('pluginProducts:unlockChooseUseShared') +
          ' → OK' +
          '\n' +
          tr('pluginProducts:unlockChooseKeepCurrent') +
          ' → Cancel',
      )
    })()
    if (choice) {
      ...
      const views = Array.isArray(viewsRaw) ? (viewsRaw as ViewRow[]) : []
      const rows: ColorMockupRow[] = Array.isArray(views[viewIndex]?.colorMockups)
        ? (views[viewIndex]!.colorMockups as ColorMockupRow[])
        : []
      let source: PrintAreaPlacement[] = []
      for (let j = 0; j < rows.length; j++) {
        if (j === activeColor) continue
        if (!rows[j]?.placementLocked) {
          source = Array.isArray(rows[j]?.printAreaPlacement)
            ? rows[j]!.printAreaPlacement
            : []
          break
        }
      }
      if (source.length === 0 && rows[0]) {
        source = Array.isArray(rows[0].printAreaPlacement)
          ? rows[0].printAreaPlacement
          : []
      }
      setValueAtPath(
        `views.${viewIndex}.colorMockups.${activeColor}.printAreaPlacement`,
        source,
      )
    }
    setValueAtPath(path, false)
  }, [setValueAtPath, viewIndex, activeColor, lockedRaw, viewsRaw, tr])
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5 py-1">
      {colorIds.map((id, j) => {
        ...
        const swatchUrl =
          doc?.swatch && typeof doc.swatch === 'object'
            ? (doc.swatch as { url?: string }).url
            : undefined
        ...
        return (
          <span
            ...
            className={
              'group flex items-center gap-1.5 rounded-md border px-1.5 py-1 text-xs transition outline-none focus-visible:ring-1 focus-visible:ring-ring ' +
              (disabled ? 'opacity-50 cursor-not-allowed ' : 'cursor-pointer ') +
              (isActive
                ? 'border-foreground/40 bg-background ring-1 ring-foreground/40'
          ...
          >
            <span
              ...
              style={
                swatchUrl
                  ? { backgroundImage: `url(${swatchUrl})`, backgroundSize: 'cover' }
                  : { backgroundColor: hex }
              }
              aria-hidden
            />
            ...
            {isActive && !disabled ? (
              <button
                ...
                title={
                  lockedRaw
                    ? tr('pluginProducts:unlockTooltip')
                    : tr('pluginProducts:lockTooltip')
                }
                aria-label={
                  lockedRaw
                    ? tr('pluginProducts:unlockPlacement')
                    : tr('pluginProducts:lockPlacement')
                }
              >
                {lockedRaw ? (
      ...
      {!disabled ? (
        // RelationshipPicker is itself popover-based, so don't wrap it in
