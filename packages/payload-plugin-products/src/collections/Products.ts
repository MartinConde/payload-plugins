import type { CollectionConfig, Field, FieldHook, Validate } from 'payload'
...
const sanitizeMockupTransform: FieldHook = ({ value }) =>
  value === undefined ? undefined : normalizeMockupTransform(value)

// Conditional fields can't safely use `required: true` (Payload still enforces
// it on hidden rows), so each view's print-area inputs predicate-validate
// against the sibling `printAreaSource`. Typed via Payload's `Validate` generic
// with the view's sibling shape so `siblingData` resolves without casts.
type ViewSibling = { printAreaSource?: 'template' | 'custom' }

const validatePrintAreaTemplate: Validate<unknown, unknown, ViewSibling> = (
  value,
  { siblingData },
) =>
  siblingData?.printAreaSource === 'template' && !value
    ? 'pluginProducts:templateRequired'
    : true

const validateCustomWidth: Validate<unknown, unknown, ViewSibling> = (value, { siblingData }) =>
  siblingData?.printAreaSource === 'custom' && (typeof value !== 'number' || value <= 0)
    ? 'pluginProducts:widthRequired'
    : true

const validateCustomHeight: Validate<unknown, unknown, ViewSibling> = (value, { siblingData }) =>
  siblingData?.printAreaSource === 'custom' && (typeof value !== 'number' || value <= 0)
    ? 'pluginProducts:heightRequired'
    : true

const sanitizePrintAreaPlacement: FieldHook = ({ value }) =>
  value === undefined ? undefined : normalizePlacements(value)
 ...
 */
export const buildProductsCollection = ({
  slug,
  mediaCollectionSlug,
  printTemplatesSlug,
  colorSwatchesSlug,
  defaultViewPresets,
  overrides,
}: BuildProductsCollectionOptions): CollectionConfig => {
  const generalFields: Field[] = [
    {
      ...
      admin: { description: productsT('pluginProducts:titleDesc') },
    },
    {
      name: 'slug',
      type: 'text',
      ...
      admin: { description: productsT('pluginProducts:slugDesc') },
    ...
    },
    {
      ...
      options: [
        { value: 'draft', label: productsT('pluginProducts:statusDraft') },
        { value: 'publish', label: productsT('pluginProducts:statusPublish') },
      ],
    },
    {
      name: 'description',
    ...
    },
    {
      ...
      admin: { description: productsT('pluginProducts:priceDesc') },
    },
    {
      name: 'colors',
      type: 'relationship',
  ...
  const viewsField: Field = {
    name: 'views',
    type: 'array',
    minRows: 1,
    label: productsT('pluginProducts:viewsLabel'),
    hooks: { beforeChange: [normalizeView] },
    fields: [
      {
        name: 'id',
        type: 'text',
        admin: { hidden: true },
        hooks: { beforeChange: [ensureRowId] },
      },
      {
        name: 'name',
        type: 'text',
      ...
      },
      {
        ...
        options: [
          { value: 'template', label: productsT('pluginProducts:sourceTemplate') },
          { value: 'custom', label: productsT('pluginProducts:sourceCustom') },
        ],
      },
      {
      ...
      // so adding/removing colors only adds/prunes the affected rows.
      {
        ...
        fields: [
          ...
          },
          {
            ...
            type: 'json',
            admin: { hidden: true },
            hooks: { beforeChange: [sanitizeMockupTransform] },
          },
          {
            name: 'printAreaPlacement',
            type: 'json',
            admin: { hidden: true },
            hooks: { beforeChange: [sanitizePrintAreaPlacement] },
          },
          {
            ...
            admin: { description: productsT('pluginProducts:placementLockedDesc') },
          },
        ],
      },
    ],
  ...
  const designerCanvasField: Field = {
    name: 'designerCanvas',
    type: 'ui',
    admin: {},
  ...
  const variantsPlaceholder: Field = {
    name: 'variantsPlaceholder',
    type: 'ui',
    admin: {},
    custom: {
      'plugin-shadcn-admin': { input: VariantsPlaceholder },
    },
  }

  const seoPlaceholder: Field = {
    name: 'seoPlaceholder',
    type: 'ui',
    admin: {},
    custom: {
      'plugin-shadcn-admin': { input: SeoPlaceholder },
    },
  }
  ...
  return {
    ...
    admin: {
      useAsTitle: 'title',
      defaultColumns: ['title', 'slug', 'status', 'price', 'updatedAt'],
    ...
    hooks: {
      ...
      ...Object.fromEntries(
        Object.entries(overrides?.hooks ?? {}).filter(
          ([k]) => !['beforeChange', 'afterRead'].includes(k),
        ),
      ),
    },
    fields: [
      {
        ...
        tabs: [
          ...
          },
          {
            ...
            fields: [viewsField, designerCanvasField],
          },
          {
            label: productsT('pluginProducts:variantsTab'),
            fields: [variantsPlaceholder],
          },
          {
    ...
    ...(overrides
      ? Object.fromEntries(
          Object.entries(overrides).filter(
            ([k]) => !['admin', 'hooks', 'fields'].includes(k),
          ),
        )
      : {}),
  }
