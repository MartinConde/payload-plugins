...
const DEFAULT_VIEW_PRESETS = ['Front', 'Back', 'Left', 'Right', 'Sleeve']
...
export const productsPlugin =
  (options: ProductsPluginConfig = {}): Plugin =>
  (config: Config): Config => {
    ...
    const printTemplatesSlug =
      options.printTemplatesSlug ?? DEFAULT_PRINT_TEMPLATES_SLUG
    const colorSwatchesSlug =
      options.colorSwatchesSlug ?? DEFAULT_COLOR_SWATCHES_SLUG
    const defaultViewPresets =
      options.defaultViewPresets && options.defaultViewPresets.length > 0
        ? options.defaultViewPresets
    ...
    const toAdd: NonNullable<Config['collections']> = []
    if (!existing.some((c) => c.slug === slug)) {
      toAdd.push(
        buildProductsCollection({
          slug,
          mediaCollectionSlug,
          printTemplatesSlug,
          colorSwatchesSlug,
    ...
    if (!existing.some((c) => c.slug === printTemplatesSlug)) {
      toAdd.push(buildPrintTemplatesCollection({ slug: printTemplatesSlug }))
    }
    if (!existing.some((c) => c.slug === colorSwatchesSlug)) {
      toAdd.push(
        buildColorSwatchesCollection({
    ...
    if (toAdd.length > 0) {
      next.collections = [...existing, ...toAdd]
    }
