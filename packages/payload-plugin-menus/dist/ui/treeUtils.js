/* Pure helpers for the flattened-tree dnd-kit pattern (the canonical "sortable
   ...
   focused on rendering. */ /** Depth-first flatten, preserving order and recording parent + depth. */ export const flattenTree = (items, parentId = null, depth = 0)=>items.reduce((acc, item, index)=>{
        acc.push({
            ...item,
            parentId,
            depth,
 ...
 *  and repopulated from each row's (possibly updated) `parentId`. */ export const buildTree = (flattened)=>{
    const root = {
        id: 'root',
        children: []
    };
 ...
 *  hide a dragged subtree and the children of collapsed rows). */ export const removeChildrenOf = (items, ids)=>{
...
const getMaxDepth = (prev)=>prev ? prev.depth + 1 : 0;
const getMinDepth = (next)=>next ? next.depth : 0;
/** Compute the projected depth + parent for the active drag, given the
 *  horizontal offset. Mirrors the dnd-kit sortable-tree example. */ export const getProjection = (items, activeId, overId, dragOffset, indentationWidth, /** Max nesting LEVELS (1 = flat). Caps the projected depth at `levelCap - 1`.
   *  Undefined = unlimited. */ levelCap)=>{
    ...
    const maxDepth = levelCap && levelCap > 0 ? Math.min(structuralMax, levelCap - 1) : structuralMax;
    ...
    const getParentId = ()=>{
        ...
        const newParent = newItems.slice(0, overItemIndex).reverse().find((i)=>i.depth === depth)?.parentId;
 ...
 *  in this pure module). */ const arrayMoveLite = (arr, from, to)=>{
