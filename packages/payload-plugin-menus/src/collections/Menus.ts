...
const sanitizeTree: FieldHook = ({ value }) =>
  value === undefined ? undefined : stripResolved(normalizeMenuTree(value))
 ...
 */
export const buildMenusCollection = ({
  slug,
  linkableCollections,
  localized,
  maxDepth,
  ...
  overrides,
}: BuildMenusCollectionOptions): CollectionConfig => ({
  slug,
  labels: {
  ...
  hooks: {
    ...overrides?.hooks,
    afterRead: [
      resolveMenuTree({ linkableCollections, resolveUrl, resolveOverrideAccess }),
  ...
  fields: [
    {
      ...
      admin: { description: menusT('pluginMenus:nameDesc') },
    },
    {
      name: 'slug',
      type: 'text',
      ...
      admin: { description: menusT('pluginMenus:slugDesc') },
    ...
    },
    {
      ...
      type: 'json',
      localized,
      hooks: { beforeChange: [sanitizeTree] },
      ...
      admin: { description: menusT('pluginMenus:treeDesc') },
      custom: {
        ...
        'plugin-shadcn-admin': { input: MenuTreeInput, linkableCollections, maxDepth },
      },
    },
    ...(overrides?.fields ?? []),
  ],
