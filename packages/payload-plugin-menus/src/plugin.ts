...
export const menusPlugin =
  (options: MenusPluginConfig = {}): Plugin =>
  (config: Config): Config => {
    if (options.disabled) return config

    const slug = options.slug ?? DEFAULT_SLUG
    const linkableCollections =
      options.linkableCollections && options.linkableCollections.length > 0
        ? options.linkableCollections
        : DEFAULT_LINKABLE
