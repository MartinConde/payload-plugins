/* Pure tree mutations (operate on the nested tree; return a new tree) plus the
   ...
   separate so MenuTreeEditor.tsx stays focused on rendering. */
...
export const patchItem = (tree: MenuTree, id: string, patch: Partial<MenuItem>): MenuTree =>
  tree.map((item) =>
    item.id === id
...
export const removeItem = (tree: MenuTree, id: string): MenuTree =>
  tree
    .filter((item) => item.id !== id)
    .map((item) => ({ ...item, children: removeItem(item.children, id) }))

export const addChild = (tree: MenuTree, parentId: string, child: MenuItem): MenuTree =>
  tree.map((item) =>
    item.id === parentId
...
export const cloneItem = (item: MenuItem): MenuItem => ({
  ...item,
  id: freshId(),
...
export const insertAfter = (tree: MenuTree, id: string, node: MenuItem): MenuTree => {
  const out: MenuTree = []
...
export const findItem = (tree: MenuTree, id: string): MenuItem | null => {
  for (const item of tree) {
...
export const fetchDocTitle = async (
  slug: string,
  id: string,
...
export const relabelFromDocs = async (
  tree: MenuTree,
  locale: string | null,
...
export const labelMap = (tree: MenuTree, out: Map<string, string> = new Map()) => {
  for (const item of tree) {
...
export const mergeKeepingLabels = (source: MenuTree, keep: MenuTree): MenuTree => {
