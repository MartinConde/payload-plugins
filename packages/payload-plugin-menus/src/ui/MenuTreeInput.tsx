...
/* The `.input` override mounted by shadcn-admin's FieldInput on the `menus`
   ...
   reaches Node — only the browser, where this client component actually runs. */

import * as React from 'react'

import type { FieldInputProps } from './adminTypes.js'

const MenuTreeEditor = React.lazy(() =>
  import('./MenuTreeEditor.js').then((m) => ({ default: m.MenuTreeEditor })),
)

export function MenuTreeInput(props: FieldInputProps): React.ReactElement {
  return (
    <React.Suspense
      fallback={
        <div
