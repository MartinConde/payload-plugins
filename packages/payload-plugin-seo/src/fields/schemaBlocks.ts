 ...
 */
export const buildSchemaBlocksField = ({
  name = 'schema',
  uploadsCollection,
  localized,
}: BuildSchemaBlocksOptions): BlocksField => {
  ...
  const labels = (key: Parameters<typeof seoT>[0]) => ({
    singular: seoT(key),
    plural: seoT(key),
  })
  ...
  const article: Block = {
    ...
    fields: [
      ...
      { name: 'headline', type: 'text', localized, label: 'Headline' },
      {
        name: 'description',
        type: 'textarea',
        localized,
  ...
  const product: Block = {
    ...
    fields: [
      { name: 'name', type: 'text', localized, label: 'Name' },
      img(),
      {
        name: 'description',
        type: 'textarea',
      ...
      },
      {
        ...
        fields: [
          ...
          },
          {
            ...
            type: 'select',
            label: 'Availability',
            options: ['InStock', 'OutOfStock', 'PreOrder', 'BackOrder'],
          },
        ],
      },
      {
    ...
    fields: [
      {
        ...
        label: 'Questions',
        fields: [
          { name: 'question', type: 'text', localized, label: 'Question' },
          { name: 'answer', type: 'textarea', localized, label: 'Answer' },
        ],
      },
    ],
  }
  ...
  const howTo: Block = {
    ...
    fields: [
      ...
      },
      {
        ...
        fields: [
          { name: 'name', type: 'text', localized, label: 'Name' },
          { name: 'text', type: 'textarea', localized, label: 'Text' },
          img(),
        ],
      },
      {
    ...
    fields: [
      ...
      { name: 'locationAddress', type: 'text', label: 'Location address' },
      {
        ...
        fields: [
          ...
          { name: 'url', type: 'text', label: seoT('pluginSeo:labelUrl') },
        ],
      },
    ],
  }

  const localBusiness: Block = {
    slug: 'localBusiness',
    ...
    fields: [
      ...
      img(),
      {
        ...
        fields: [
          ...
          { name: 'priceRange', type: 'text', label: 'Price range (e.g. $$)' },
        ],
      },
      {
        name: 'address',
      ...
      },
      {
        ...
        fields: [
          ...
          },
          {
            type: 'row',
            fields: [
              { name: 'opens', type: 'text', label: 'Opens (e.g. 09:00)' },
              { name: 'closes', type: 'text', label: 'Closes (e.g. 17:00)' },
            ],
          },
        ],
      },
  ...
  const recipe: Block = {
    ...
    fields: [
      ...
      },
      {
        type: 'row',
        fields: [
          { name: 'prepTime', type: 'text', label: 'Prep time (ISO 8601)' },
          { name: 'cookTime', type: 'text', label: 'Cook time (ISO 8601)' },
          { name: 'recipeYield', type: 'text', label: 'Yield (e.g. 4 servings)' },
        ],
      },
      {
        name: 'ingredients',
        type: 'array',
        label: 'Ingredients',
        fields: [{ name: 'item', type: 'text', localized, label: 'Item' }],
      },
      {
        name: 'instructions',
        type: 'array',
        label: 'Instructions',
        fields: [{ name: 'text', type: 'textarea', localized, label: 'Step' }],
      },
    ],
  }
  ...
  const video: Block = {
    ...
    fields: [
      ...
      { name: 'uploadDate', type: 'date', label: 'Upload date' },
      {
        type: 'row',
        fields: [
          { name: 'contentUrl', type: 'text', label: 'Content URL' },
          { name: 'embedUrl', type: 'text', label: 'Embed URL' },
        ],
      },
      { name: 'duration', type: 'text', label: 'Duration (ISO 8601, e.g. PT2M30S)' },
    ],
  }
    ...
    fields: [
      {
        ...
        admin: { description: seoT('pluginSeo:jsonLdDesc') },
      },
    ],
  }
  ...
  return {
    ...
    admin: { description: seoT('pluginSeo:schemaDesc') },
    blocks: [
      article,
      product,
      faq,
