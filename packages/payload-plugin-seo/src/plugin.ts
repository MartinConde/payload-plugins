...
export const seoPlugin =
  ...
  (config: Config): Config => {
    ...
    const tabLabel =
      (typeof tabOption === 'object' && tabOption.label) || 'SEO'
    ...
    const hasMetaField = (fields: Field[] = []): boolean =>
      fields.some((f) => {
        if ('name' in f && f.name === fieldName) return true
        if (f.type === 'tabs') {
          return f.tabs.some((t) =>
            ('fields' in t ? t.fields : []).some(
              (tf) => 'name' in tf && tf.name === fieldName,
            ),
          )
        }
        return false
    ...
    const placeMetaField = (fields: Field[]): Field[] => {
      if (!tabOption) {
        return [
          ...fields,
          buildMetaField({
      ...
      const seoTab = { label: tabLabel, fields: [metaField] }
      const tabsIdx = fields.findIndex((f) => f.type === 'tabs')
      if (tabsIdx === -1) {
        return [...fields, { type: 'tabs', tabs: [seoTab] }]
      }
      return fields.map((f, i) =>
        i === tabsIdx && f.type === 'tabs'
          ? { ...f, tabs: [...f.tabs, seoTab] }
          : f,
      )
    }
    ...
    if (metaTargets.size > 0) {
      next.collections = (config.collections ?? []).map((collection) =>
        metaTargets.has(collection.slug) && !hasMetaField(collection.fields)
          ? { ...collection, fields: placeMetaField(collection.fields) }
          : collection,
      )
    }
    ...
    if (globalTargets.size > 0) {
      next.globals = (next.globals ?? config.globals ?? []).map((global) =>
        globalTargets.has(global.slug) && !hasMetaField(global.fields)
          ? { ...global, fields: placeMetaField(global.fields) }
          : global,
      )
    }
    ...
    if (settings !== false) {
      const slug =
        (typeof settings === 'object' && settings.slug) || DEFAULT_SETTINGS_SLUG
      ...
      if (!existing.some((g) => g.slug === slug)) {
        ...
        const templateCollections =
          options.collections && options.collections.length > 0
            ? options.collections
            : (next.collections ?? config.collections ?? []).map((c) => c.slug)
        next.globals = [
          ...existing,
    ...
    if (redirects !== false) {
      const slug =
        (typeof redirects === 'object' && redirects.slug) ||
        DEFAULT_REDIRECTS_SLUG
      ...
      if (!existing.some((c) => c.slug === slug)) {
        ...
        const targetCollections =
          (typeof redirects === 'object' && redirects.collections) ||
          (options.collections && options.collections.length > 0
            ? options.collections
        ...
        next.collections = [
          ...
          buildRedirectsCollection({
            ...
            overrides:
              typeof redirects === 'object' ? redirects.overrides : undefined,
          }),
        ]
      }
    }
    ...
    if ((options.wizard ?? true) && settingsEnabled) {
      const settingsSlug =
        (typeof options.settingsGlobal === 'object' &&
          options.settingsGlobal.slug) ||
        DEFAULT_SETTINGS_SLUG
      const redirectsSlug =
        (typeof options.redirects === 'object' && options.redirects.slug) ||
        DEFAULT_REDIRECTS_SLUG

      next.custom = {
        ...(next.custom ?? {}),
        'plugin-seo': { settingsSlug, uploadsCollection, fieldName, redirectsSlug },
      }
      next.admin = {
        ...next.admin,
        components: {
