 ...
 *  `fieldName` at the top level, or one level inside a top-level tabs field. */ const hasMetaGroup = (fields = [], fieldName)=>fields.some((f)=>{
        ...
        if (f.type === 'tabs') {
            return f.tabs.some((tab)=>('fields' in tab ? tab.fields : []).some((tf)=>'name' in tf && tf.name === fieldName && tf.type === 'group'));
        }
        return false;
    });
/* RSC for the SEO setup wizard, registered at
   ...
   shared `ViewShell` chrome. Mounted as `payload-plugin-seo/rsc#SeoWizardView`. */ export async function SeoWizardView(props) {
    const { initPageResult } = props;
    const { req } = initPageResult;
    const { payload, user, i18n } = req;
    const custom = {
        ...DEFAULTS,
        ...payload.config.custom?.['plugin-seo'] ?? {}
    };
    ...
    const defaultLocale = typeof payload.config.localization === 'object' ? payload.config.localization.defaultLocale : undefined;
    ...
    const seoCollections = (payload.config.collections ?? []).filter((c)=>hasMetaGroup(c.fields, fieldName)).slice(0, MAX_AUDITED_COLLECTIONS);
    ...
    const collections = (await Promise.all(seoCollections.map(async (c)=>{
        try {
            const [{ totalDocs: total }, { totalDocs: missing }] = await Promise.all([
                payload.count({
                    collection: c.slug,
                    overrideAccess: false,
                    req,
            ...
            const label = plural && typeof plural !== 'function' ? getTranslation(plural, i18n) : c.slug;
            return {
                slug: c.slug,
                label,
                total,
    ...
    return /*#__PURE__*/ _jsx(ViewShell, {
        breadcrumbs: [
            {
                label: i18n.t('pluginSeo:wizardTitle')
            }
        ],
        headerActions: /*#__PURE__*/ _jsxs("a", {
            ...
            children: [
                ...
                i18n.t('pluginSeo:wizardExit')
            ]
        }),
        children: /*#__PURE__*/ _jsx(SeoWizardClient, {
